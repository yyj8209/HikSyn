...
UINT CDlgPlayRemoteFileMedical::GetMedicalFileThread(LPVOID pParam)
{
	...
	while (g_pDlgRemoteFile->m_bQuit)
	{
		...
		{			
            ...
            if (struFileInfo.byBigFileType == 0 || (struFileInfo.byBigFileType == 1 && struFileInfo.dwTotalLenH ==0)) //普通文件搜索或者大文件搜索高32字节为0
            {
            ...
            else if (struFileInfo.byBigFileType == 1 && struFileInfo.dwTotalLenH>0)  //1为大文件搜索 
            {
                if (struFileInfo.dwTotalLenH / 1024 == 0)
                {
                    csTmp.Format("%dT", struFileInfo.dwTotalLenH);
                }
                else if (struFileInfo.dwTotalLenH / 1024 > 0 && struFileInfo.dwTotalLenH / (1024 * 1024) == 0)
                {
                    csTmp.Format("%dP", struFileInfo.dwTotalLenH / 1024);
                }
                else// if ()
                {
                    csTmp.Format("%dE", struFileInfo.dwTotalLenH / 1024 / 1024);
                }
            }
