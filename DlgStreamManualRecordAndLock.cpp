...
CDlgStreamManualRecordAndLock::CDlgStreamManualRecordAndLock(CWnd* pParent /*=NULL*/)
    ...
    , m_byISO8601Enabled(FALSE)
    , m_iHourDiffWithUTC(0)
    , m_iMinuteDiffWithUTC(0)
    , m_iHourDiffWithUTCReturn(0)
    , m_iMinuteDiffWithUTCReturn(0)
{
...
void CDlgStreamManualRecordAndLock::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Check(pDX, IDC_CHECK_ISO8601_ENABLED, m_byISO8601Enabled);
    DDX_Text(pDX, IDC_EDIT_TIMEDIFFERENCE_H, m_iHourDiffWithUTC);
    DDX_Text(pDX, IDC_EDIT_TIMEDIFFERENCE_M, m_iMinuteDiffWithUTC);
    DDX_Text(pDX, IDC_EDIT_TIMEDIFFERENCE_H_RETURN, m_iHourDiffWithUTCReturn);
    DDX_Text(pDX, IDC_EDIT_TIMEDIFFERENCE_M_RETURN, m_iMinuteDiffWithUTCReturn);
}
...
// 	}
    ...
    if (true== m_byISO8601Enabled)
    {
        struTimeLock.byISO8601 = 1;
        struTimeLock.cTimeDifferenceH = m_iHourDiffWithUTC;
        struTimeLock.cTimeDifferenceM=m_iMinuteDiffWithUTC;
    }
    else
    {
        struTimeLock.byISO8601 = 0;
        struTimeLock.cTimeDifferenceH = 0;
        struTimeLock.cTimeDifferenceM = 0;
    }
	...
	if (NET_DVR_LockStreamFileByTime(m_lLoginID, &struTimeLock, &struLockReturn))
	{
        ...
        if (1 == struLockReturn.byISO8601)
        {
            m_iHourDiffWithUTCReturn = struLockReturn.cTimeDifferenceH;
            m_iMinuteDiffWithUTCReturn = struLockReturn.cTimeDifferenceM;
        }
        else if (0 == struLockReturn.byISO8601)
        {
            m_iHourDiffWithUTCReturn = 0;
            m_iMinuteDiffWithUTCReturn = 0;
        }
