...
void CDlgAlarmHostRs485::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMBO_OUTPUT_TYPE, m_comboOutputType);
    DDX_Text(pDX, IDC_EDIT_AIR_NO, m_airConditionNo);
}
...
void CDlgAlarmHostRs485::InitOutputType()
{
    CString tmp;
    char szLan[128] = { 0 };
    m_comboOutputType.ResetContent();

    g_StringLanType(szLan, "无效", "Invalid");
    m_comboOutputType.AddString(szLan);
    m_comboOutputType.SetItemData(0, 0);

    g_StringLanType(szLan, "输出卡号", "Output CardNo");
    m_comboOutputType.AddString(szLan);
    m_comboOutputType.SetItemData(1, 1);

    g_StringLanType(szLan, "输出工号", "Output EmployeeNo");
    m_comboOutputType.AddString(szLan);
    m_comboOutputType.SetItemData(2, 2);
}
...
void CDlgAlarmHostRs485::GetRs485CfgFromWnd(NET_DVR_ALARM_RS485CFG &struRs485Cfg)
{
    ...
    struRs485Cfg.byOutputDataType = m_comboOutputType.GetCurSel();

    if (m_cmbMode.GetCurSel() == 5)
    {
        struRs485Cfg.byMode = 0xff;
    }
    else
    {
...
void CDlgAlarmHostRs485::SetRs485CfgToWnd(NET_DVR_ALARM_RS485CFG &struRs485Cfg)
{
    ...
    m_comboOutputType.SetCurSel(struRs485Cfg.byOutputDataType);

    if (struRs485Cfg.byMode == 0xff)
    {
        m_cmbMode.SetCurSel(5);
    }
    else
    {
...
void CDlgAlarmHostRs485::OnBtnSetAircondition() 
{
    ...
    struAirCondition.byAirConditionNo = m_airConditionNo;
...
void CDlgAlarmHostRs485::InitModeCombo()
{
	...
	m_cmbMode.AddString(szLan);
    g_StringLanType(szLan, "连接梯控主机", "Connect elevator host");
    m_cmbMode.AddString(szLan);
    g_StringLanType(szLan, "禁用", "Disabled");
    m_cmbMode.AddString(szLan);
    m_cmbMode.SetItemData(5, 255);
}
